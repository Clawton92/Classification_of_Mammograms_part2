...
def add_model_head(base_model, n_categories, activation_function):
    """
    Takes a base model and adds a pooling and a output based the activation function

    Args:
        base_model (keras Sequential model): model to attach head to
        n_categories (int): number of image classes
...
def create_transfer_model(input_size, n_categories, activation_function, weights = 'imagenet', model=InceptionV3):
    ...
    Args:
        ...
        ***Note that transfer models in keras will only accept an input shape with three channels.
        ...
        n_categories (int): number of classes (labels)
        activation_function (string): activation function for the last dense layer (classification)
        weights (str or arg): weights to use for model (typically use imagenet)
...
def print_model_properties(model, indices = 0):
    ...
    args:
    model (keras model): A pretrained Keras model from keras applications
    indices (int): The index of where to start printing the layers

    returns:
        print of all layers and layer info.
    '''
